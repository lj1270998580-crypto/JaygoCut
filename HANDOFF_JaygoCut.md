...
- 修复导出剪映草稿的旧审核页兼容问题：新页面内置 `buildTimeMapper`，旧 `review.html` 由 `review_server.js` 自动注入兼容脚本。
- 修复素材生成中断后的状态恢复：无产物的 `generating` 图片/视频会恢复为 `queued` 并提示可重新生成。
- Agnes 请求限流统一为图片、视频创建、视频轮询共用 3.2 秒间隔，控制在 RPM20 以下。
- Agnes 视频生成优先使用 `/agnesapi?video_id=...` 轮询，并兼容 `remixed_from_video_id` 作为任务 ID。
- AI 管家支持本地执行常用操作：导出剪映、重新生成指定图片/视频、生成配图、生成视频素材、AI 分析、应用或清除 AI 建议。
- 审核页文案从 LLM 标记升级为 AI 分析，提示词强调先理解全片结构，再按语气词、重复句、跑题、非主讲干扰等维度保守标记。
- 回归测试覆盖旧审核页兼容补丁、生成中断恢复、Agnes RPM 限流、AI 管家入口、视频数量自动匹配和剪映导出忙碌状态。
...
gh release create v0.1.18 `
  "dist\JaygoCut-Setup-0.1.18.exe" `
  "dist\JaygoCut-Setup-0.1.18.exe.blockmap" `
  "dist\latest.yml" `
  "release-notes.json" `
  --title "JaygoCut v0.1.18" `
