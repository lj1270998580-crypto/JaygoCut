...
桌面安装包：`C:\Users\15119\Desktop\JaygoCut-Setup-0.1.12.exe`
...
- 修复火山引擎和阿里 Qwen3-ASR 连通检测不稳定：云端 ASR 成功接收测试任务即判定连通，查询接口短暂失败只作为警告，不再误报失败。
- 阿里 Qwen3-ASR 连通检测增加超时、重试和异步任务状态容错。
- 火山 ASR 连通检测改为真实提交小音频测试任务，并保留上传服务健康检查作为辅助诊断。
- 审核页顶部改为“核心固定 + 工具折叠”：播放、清空、执行裁剪、状态和删除时长统计固定在一行，其它工具默认折叠。
- 审核页新增边界精修参数：字头提前、字尾延后、语气词加强、静音保护，解决少部分字头/语气词残留音。
- 新增转录质量检测：识别 `���`、无效时间戳、倒序时间戳、空白文字和超长空白段，并在审核页提示。
- 新增回归测试：覆盖静音阈值等于 0.2、转录质量检测、删除边界补偿。
...
curl.exe -I --max-time 30 https://ailabing.cn/downloads/jaygo/JaygoCut-Setup-0.1.12.exe
```
...
- ASR 连通检测不要要求完整转录文本返回；阿里和火山都是异步任务，任务提交成功即可说明 Key、模型和基础接口连通。
