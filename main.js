...
async function resumeReviewFromHistory(entry) {
  if (!entry || !entry.reviewDir || !entry.videoPath) {
    throw new Error('历史记录无效');
  }
  if (!fs.existsSync(entry.reviewDir)) {
    throw new Error(`审核目录不存在：${entry.reviewDir}`);
  }
  if (!fs.existsSync(entry.videoPath)) {
    throw new Error(`视频文件不存在：${entry.videoPath}`);
  }
  ...
  await rebuildReviewHtml(entry.reviewDir, runtimeEnv);
  standaloneReviewServer = startReviewServer(
    entry.reviewDir,
    entry.videoPath,
    entry.outputRoot || path.dirname(entry.projectDir || entry.reviewDir),
    port,
    runtimeEnv,
  );
