...
| 视频配图 | 审核页可用 LLM 分析内容生成配图点、分镜提示词和图片预览。 |
| 剪映导出 | 审核页可导出 SRT、TXT 和完整剪映草稿；完整草稿会自动识别剪映/CapCut 草稿目录并直接写入，打开剪映即可看到。也支持基于用户选定的模板草稿生成新草稿，原模板不会被覆盖。 |
...
{
  "version": "0.1.16",
  "notes": "- 完整剪映草稿导出支持自动识别剪映/CapCut 草稿目录\n- 支持基于用户选定模板草稿生成新草稿，不覆盖原模板\n- 优化剪映草稿导出结果提示和回归测试"
}
```
