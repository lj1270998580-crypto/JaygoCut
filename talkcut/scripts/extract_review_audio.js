...
const sourceAudioStartSec = probeFloat(
  `${shellQuote(FFPROBE_BIN)} -v error -select_streams a:0 -show_entries stream=start_time -of csv=p=0 ${shellQuote(fileArg(INPUT))}`
);
...
if (sourceAudioStartSec > 0) {
  ...
  command = [
    `${shellQuote(FFMPEG_BIN)} -y -i "${fileArg(INPUT)}"`,
