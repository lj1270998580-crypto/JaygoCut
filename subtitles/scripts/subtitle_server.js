...
if (fs.existsSync(SUBTITLES_FILE)) {
  ...
  console.log(`📝 加载 ${subtitles.length} 条subtitles`);
} else {
  ...
  if (req.method === 'POST' && req.url === '/api/burn') {
    ...
    req.on('end', () => {
      try {
        ...
        const readablePath = './3_输出/' + baseName + '_subtitles稿.md';
        fs.writeFileSync(readablePath, readable);
        console.log('📝 已保存subtitles稿:', readablePath);
        ...
        const outputPath = './3_输出/' + baseName + '_subtitles.mp4';
  ...
  <div class="container">
    <div class="video-panel">
      ...
      <div class="controls">
        ...
        <button class="btn-secondary" onclick="saveSubtitles()">💾 保存subtitles</button>
        <button class="btn-secondary" onclick="saveSRT()">📄 导出 SRT</button>
        <button class="btn-primary" onclick="burnSubtitles()">🎬 烧录subtitles</button>
    ...
    <div class="subtitle-panel">
      <div class="subtitle-header">
        <h2>subtitles列表 (<span id="count">0</span>)</h2>
